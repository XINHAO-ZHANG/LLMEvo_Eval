#!/bin/bash
# Tsp30 批量实验（可与 batch_run_tsp60.sh 并行： ./batch_run_tsp30.sh & ./batch_run_tsp60.sh & wait）

LOG_DIR="logs/batch_run_tsp30_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"
...
log_info "=== Tsp30 done: $COMPLETED_EXPERIMENTS/$TOTAL_EXPERIMENTS, ${D}s ==="
echo "Tsp30: $COMPLETED_EXPERIMENTS/$TOTAL_EXPERIMENTS, ${D}s" > "$LOG_DIR/summary.txt"
